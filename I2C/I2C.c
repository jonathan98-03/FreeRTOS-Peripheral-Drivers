...
typedef struct {
  ...
  // Data Needed by Slave operations
...
static bool i2c__handle_state_machine(i2c_s *i2c) {
  enum {
    ...
    I2C__STATE_SR_SLA_W_RCV = 0x60,
    I2C__STATE_SR_DATA_RCV = 0x80,
    I2C__STATE_SR_SLAVE_NACK_SENT = 0xA0,

    // Slave Transmitter Mode (ST)
    I2C__STATE_ST_SLA_R_RCV = 0xA8,
    I2C__STATE_ST_MASTER_DATA_SENT = 0xB8,
    I2C__STATE_ST_LAST_DATA_SENT = 0xC0,
  ...
  // Slave Receiver Operations
  case I2C__STATE_SR_SLA_W_RCV:
    i2c->index_location = 0;
    i2c->received_first_byte = false;
    i2c__set_ack_flag(lpc_i2c);
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);

  case I2C__STATE_SR_DATA_RCV:
    if (i2c->received_first_byte) {
      if (i2c_slave_callback__write_memory(i2c->index_location, lpc_i2c->DAT)) {
        i2c->index_location++;
      }
    } else {
      i2c->received_first_byte = true;
      i2c->index_location = lpc_i2c->DAT;
    }
    i2c__set_ack_flag(lpc_i2c);
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    break;
  ...
  // Slave Transmitter Operations
  case I2C__STATE_ST_SLA_R_RCV: // 0xA8
    if (i2c_slave_callback__read_memory(i2c->index_location, &lpc_i2c->DAT)) {
      i2c->index_location++;
      i2c__set_ack_flag(lpc_i2c);
      i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    }
  ...
  case I2C__STATE_ST_MASTER_DATA_SENT: // 0xB8
    if (i2c_slave_callback__read_memory(i2c->index_location, &lpc_i2c->DAT)) {
      i2c->index_location++;
      // i2c__set_ack_flag(lpc_i2c);
      i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
  ...
  case I2C__STATE_ST_LAST_DATA_SENT: // 0xC0
    fprintf(stderr, "I am in last data \n");
    // i2c__set_ack_flag(lpc_i2c);
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    break;
