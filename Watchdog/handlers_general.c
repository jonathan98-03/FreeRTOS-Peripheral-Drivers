...
app_cli_status_e cli__watchdog(app_cli__argument_t argument, sl_string_t user_input_minus_command_name,
                               app_cli__print_string_function cli_output) {

  // local string variable
  sl_string_t s = user_input_minus_command_name;

  // check first word of string
  if (sl_string__begins_with_ignore_case(s, "suspend")) {

    // extract first word
    sl_string__erase_first_word(s, ' ');

    // gets task handler, by task name
    TaskHandle_t task_handle = xTaskGetHandle(s);

    // api will search for taskname
    // if invalid string, print unknown

    if (NULL == task_handle) {

      sl_string__insert_at(s, 0, "Unknown Task:");
      cli_output(NULL, s);
    } else {

      // if found name suspend task handle I passed
      vTaskSuspend(task_handle);
    }

    // checks first word of string
    ...
    // if wrong command it will output this
