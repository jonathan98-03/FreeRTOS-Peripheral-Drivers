...
void adc__enable_burst_mode(void) {
  const uint32_t start_conversion_mask = (7 << 24); // 3bits - B26:B25:B24
  const uint32_t burst = (1 << 16);

  LPC_ADC->CR &= ~start_conversion_mask; // clear register START before enabling burst
  LPC_ADC->CR |= burst | (1 << 5);
}
...
uint16_t adc__get_channel_reading_with_burst_mode(uint8_t channel_number) {
  ...
  // fprintf(stderr, "This is %u \n", LPC_ADC->STAT);
  // while (!(LPC_ADC->DR[channel_number] & (1 << channel_number))) // wait till conversion is complete
  // {
  //   ;
  // }
